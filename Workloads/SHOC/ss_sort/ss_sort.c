...
void init(int bucket[BUCKETSIZE])
{
    ...
    for (radixID = 0; radixID < BUCKETSIZE; ++radixID)
        bucket[radixID] = 0;
}
...
void ss_sort()
{
    ...
    for (exp = 0; exp < 2; exp+=2){
        ...
        if (flag == 0)
        {
            update(b, bucket, arr, exp);
            flag = 1;
        }
        else
        {
            update(arr, bucket, b, exp);
            flag = 0;
        }
...
int main()
{
    ...
    for(i=0; i<N; i++){ 
        arr[i] = (TYPE)(((double) rand() / (RAND_MAX)) * (max-min) + min);
    }
