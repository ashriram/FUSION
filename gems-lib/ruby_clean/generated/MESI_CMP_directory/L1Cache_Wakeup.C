      ...
      if ((((*(m_chip_ptr->m_L1Cache_responseToL1Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
            ...
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L1cache.sm:260, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
...
;
                                    ...
                                    cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L1cache.sm:283, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("Invalid L1 response type") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
      ...
      if ((((*(m_chip_ptr->m_L1Cache_requestToL1Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
                    if (ASSERT_FLAG && !(((((*in_msg_ptr)).m_Destination).isElement(m_machineID)))) {
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L1cache.sm:293, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
...
;
                                    ...
                                    cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L1cache.sm:304, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("Invalid forwarded request type") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
