...
L1Cache_State L1Cache_Controller::L1Cache_getState(Address param_addr)
{
  if ((((((*(m_chip_ptr->m_L1Cache_L1DcacheMemory_vec[m_version]))).isTagPresent(param_addr)) && (((*(m_chip_ptr->m_L1Cache_L1IcacheMemory_vec[m_version]))).isTagPresent(param_addr))) == (true))) {
    DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L1cache.sm:201: ", m_chip_ptr->getID());
;
    DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L1cache.sm:202: ", param_addr);
;
  }
    ...
    cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L1cache.sm:204, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
