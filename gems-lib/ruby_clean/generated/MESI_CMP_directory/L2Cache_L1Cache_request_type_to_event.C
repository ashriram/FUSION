      ...
      if ((param_type == CoherenceRequestType_GETX)) {
        ...
        if ((param_type == CoherenceRequestType_UPGRADE)) {
          ...
          if ((param_type == CoherenceRequestType_PUTX)) {
            ...
            DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:289: ", param_addr);
;
            DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:290: ", param_type);
;
                        cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:291, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("Invalid L1 forwarded request type") << ", PID: " << getpid() << endl;
