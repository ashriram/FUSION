      ...
      if ((((*(m_chip_ptr->m_Directory_requestToDir_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
            ...
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-mem.sm:110, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
...
;
                            ...
                            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-mem.sm:116, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("Invalid message") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
      ...
      if ((((*(m_chip_ptr->m_Directory_responseToDir_vec[m_version]))).isReady())) {
        {
          ...
          in_msg_ptr = dynamic_cast<const ResponseMsg*>(((*(m_chip_ptr->m_Directory_responseToDir_vec[m_version]))).peek());
          assert(in_msg_ptr != NULL);
                    if (ASSERT_FLAG && !(((((*in_msg_ptr)).m_Destination).isElement(m_machineID)))) {
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-mem.sm:125, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
...
;
            ...
            DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-mem.sm:133: ", ((*in_msg_ptr)).m_Type);
;
                        cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-mem.sm:134, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("Invalid message") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
