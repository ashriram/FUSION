...
void Directory_Controller::a_sendAck(const Address& addr)
{
  DEBUG_MSG(GENERATED_COMP, HighPrio,"executing");
  {
    ...
    assert(in_msg_ptr != NULL);
    {
      ...
      ((*(m_chip_ptr->m_Directory_responseFromDir_vec[m_version]))).enqueue(out_msg, MEM_to_L2_MSG_LATENCY);
    }
  }
}
...
void Directory_Controller::d_sendData(const Address& addr)
{
  DEBUG_MSG(GENERATED_COMP, HighPrio,"executing");
  {
    ...
    assert(in_msg_ptr != NULL);
    {
      ...
      ((*(m_chip_ptr->m_Directory_responseFromDir_vec[m_version]))).enqueue(out_msg, MEM_to_L2_DATA_LATENCY);
    }
  }
}
...
void Directory_Controller::m_writeDataToMemory(const Address& addr)
{
  DEBUG_MSG(GENERATED_COMP, HighPrio,"executing");
  {
    ...
    DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-mem.sm:178: ", ((*in_msg_ptr)).m_Address);
;
    DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-mem.sm:179: ", ((*in_msg_ptr)).m_DataBlk);
;
  }
}
