      ...
      if ((((*(m_chip_ptr->m_L2Cache_unblockToL2Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
            ...
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:306, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
...
;
                            ...
                            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:312, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("unknown unblock message") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
      ...
      if ((((*(m_chip_ptr->m_L2Cache_responseToL2Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
                    if (ASSERT_FLAG && !(((((*in_msg_ptr)).m_Destination).isElement(m_machineID)))) {
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:323, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
...
;
                                ...
                                cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:338, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("unknown message type") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
...
;
                                ...
                                cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:347, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("unknown message type") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
      ...
      if ((((*(m_chip_ptr->m_L2Cache_L1RequestToL2Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
          DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:358: ", ((*in_msg_ptr)).m_Address);
;
          DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:359: ", m_chip_ptr->getID());
;
          DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:360: ", (L2Cache_getState(((*in_msg_ptr)).m_Address)));
;
          DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:361: ", ((*in_msg_ptr)).m_Requestor);
;
          DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:362: ", ((*in_msg_ptr)).m_Type);
;
          DEBUG_SLICC(MedPrio, "../protocols/FUSION/MESI_CMP_directory-L2cache.sm:363: ", ((*in_msg_ptr)).m_Destination);
;
                    if (ASSERT_FLAG && !(((machineIDToMachineType(((*in_msg_ptr)).m_Requestor)) == MachineType_L1Cache))) {
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:364, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();

          }
;
                    if (ASSERT_FLAG && !(((((*in_msg_ptr)).m_Destination).isElement(m_machineID)))) {
            cerr << "Runtime Error at ../protocols/FUSION/MESI_CMP_directory-L2cache.sm:365, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
