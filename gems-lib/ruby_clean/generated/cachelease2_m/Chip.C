...
Chip::Chip(NodeID id, Network* net_ptr):AbstractChip(id, net_ptr)
{
  ...
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_responseFromL2Cache_vec[i] = m_net_ptr->getToNetQueue(i+m_id*RubyConfig::numberOfL2CachePerChip()+MachineType_base_number(string_to_MachineType("L2Cache")), true, 6);
    assert(m_L2Cache_responseFromL2Cache_vec[i] != NULL);
    m_L2Cache_responseFromL2Cache_vec[i]->setOrdering(true);
    if (FINITE_BUFFERING) {
  ...
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_L1RequestToL2Cache_vec[i] = m_net_ptr->getFromNetQueue(i+m_id*RubyConfig::numberOfL2CachePerChip()+MachineType_base_number(string_to_MachineType("L2Cache")), true, 5);
  ...
  // L2Cache_requestFromL2TCache
  m_L2Cache_requestFromL2TCache_vec.setSize(RubyConfig::numberOfL2CachePerChip(m_id));
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_requestFromL2TCache_vec[i] = m_net_ptr->getToNetQueue(i+m_id*RubyConfig::numberOfL2CachePerChip()+MachineType_base_number(string_to_MachineType("L2Cache")), false, 1);
    assert(m_L2Cache_requestFromL2TCache_vec[i] != NULL);
    m_L2Cache_requestFromL2TCache_vec[i]->setOrdering(false);
    if (FINITE_BUFFERING) {
      m_L2Cache_requestFromL2TCache_vec[i]->setSize(PROTOCOL_BUFFER_SIZE);
    }
  m_L2Cache_requestFromL2TCache_vec[i]->setDescription("[Chip " + int_to_string(getID()) + " " + int_to_string(i) + ", L2Cache, requestFromL2TCache]");

  }
  // L2Cache_responseFromL2TCache
  m_L2Cache_responseFromL2TCache_vec.setSize(RubyConfig::numberOfL2CachePerChip(m_id));
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_responseFromL2TCache_vec[i] = m_net_ptr->getToNetQueue(i+m_id*RubyConfig::numberOfL2CachePerChip()+MachineType_base_number(string_to_MachineType("L2Cache")), false, 3);
    assert(m_L2Cache_responseFromL2TCache_vec[i] != NULL);
    m_L2Cache_responseFromL2TCache_vec[i]->setOrdering(false);
    if (FINITE_BUFFERING) {
      m_L2Cache_responseFromL2TCache_vec[i]->setSize(PROTOCOL_BUFFER_SIZE);
    }
  m_L2Cache_responseFromL2TCache_vec[i]->setDescription("[Chip " + int_to_string(getID()) + " " + int_to_string(i) + ", L2Cache, responseFromL2TCache]");

  }
  // L2Cache_unblockFromL2TCache
  m_L2Cache_unblockFromL2TCache_vec.setSize(RubyConfig::numberOfL2CachePerChip(m_id));
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_unblockFromL2TCache_vec[i] = m_net_ptr->getToNetQueue(i+m_id*RubyConfig::numberOfL2CachePerChip()+MachineType_base_number(string_to_MachineType("L2Cache")), false, 4);
    assert(m_L2Cache_unblockFromL2TCache_vec[i] != NULL);
    m_L2Cache_unblockFromL2TCache_vec[i]->setOrdering(false);
    if (FINITE_BUFFERING) {
      m_L2Cache_unblockFromL2TCache_vec[i]->setSize(PROTOCOL_BUFFER_SIZE);
    }
  m_L2Cache_unblockFromL2TCache_vec[i]->setDescription("[Chip " + int_to_string(getID()) + " " + int_to_string(i) + ", L2Cache, unblockFromL2TCache]");

  }
  // L2Cache_requestToL2TCache
  m_L2Cache_requestToL2TCache_vec.setSize(RubyConfig::numberOfL2CachePerChip(m_id));
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_requestToL2TCache_vec[i] = m_net_ptr->getFromNetQueue(i+m_id*RubyConfig::numberOfL2CachePerChip()+MachineType_base_number(string_to_MachineType("L2Cache")), false, 1);
    assert(m_L2Cache_requestToL2TCache_vec[i] != NULL);
    m_L2Cache_requestToL2TCache_vec[i]->setOrdering(false);
    if (FINITE_BUFFERING) {
      m_L2Cache_requestToL2TCache_vec[i]->setSize(PROTOCOL_BUFFER_SIZE);
    }
  m_L2Cache_requestToL2TCache_vec[i]->setDescription("[Chip " + int_to_string(getID()) + " " + int_to_string(i) + ", L2Cache, requestToL2TCache]");

  }
  // L2Cache_responseToL2TCache
  m_L2Cache_responseToL2TCache_vec.setSize(RubyConfig::numberOfL2CachePerChip(m_id));
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_responseToL2TCache_vec[i] = m_net_ptr->getFromNetQueue(i+m_id*RubyConfig::numberOfL2CachePerChip()+MachineType_base_number(string_to_MachineType("L2Cache")), false, 3);
    assert(m_L2Cache_responseToL2TCache_vec[i] != NULL);
    m_L2Cache_responseToL2TCache_vec[i]->setOrdering(false);
    if (FINITE_BUFFERING) {
      m_L2Cache_responseToL2TCache_vec[i]->setSize(PROTOCOL_BUFFER_SIZE);
    }
  m_L2Cache_responseToL2TCache_vec[i]->setDescription("[Chip " + int_to_string(getID()) + " " + int_to_string(i) + ", L2Cache, responseToL2TCache]");
  ...
  // L2Cache_L2ExpireTriggerQueue
  m_L2Cache_L2ExpireTriggerQueue_vec.setSize(RubyConfig::numberOfL2CachePerChip(m_id));
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_L2ExpireTriggerQueue_vec[i] = new MessageBuffer(this);

    assert(m_L2Cache_L2ExpireTriggerQueue_vec[i] != NULL);
    m_L2Cache_L2ExpireTriggerQueue_vec[i]->setOrdering(false);
    if (FINITE_BUFFERING) {
      m_L2Cache_L2ExpireTriggerQueue_vec[i]->setSize(PROCESSOR_BUFFER_SIZE);
    }
  m_L2Cache_L2ExpireTriggerQueue_vec[i]->setDescription("[Chip " + int_to_string(getID()) + " " + int_to_string(i) + ", L2Cache, L2ExpireTriggerQueue]");
  ...
  for (int i = 0; i < RubyConfig::numberOfL1CachePerChip(m_id); i++)  {
    m_L1Cache_requestFromL1Cache_vec[i] = m_net_ptr->getToNetQueue(i+m_id*RubyConfig::numberOfL1CachePerChip()+MachineType_base_number(string_to_MachineType("L1Cache")), true, 5);
  ...
  for (int i = 0; i < RubyConfig::numberOfL1CachePerChip(m_id); i++)  {
    m_L1Cache_responseToL1Cache_vec[i] = m_net_ptr->getFromNetQueue(i+m_id*RubyConfig::numberOfL1CachePerChip()+MachineType_base_number(string_to_MachineType("L1Cache")), true, 6);
    assert(m_L1Cache_responseToL1Cache_vec[i] != NULL);
    m_L1Cache_responseToL1Cache_vec[i]->setOrdering(true);
    if (FINITE_BUFFERING) {
  ...
  for (int i = 0; i < RubyConfig::numberOfDirectoryPerChip(m_id); i++)  {
    m_Directory_requestToDir_vec[i] = m_net_ptr->getFromNetQueue(i+m_id*RubyConfig::numberOfDirectoryPerChip()+MachineType_base_number(string_to_MachineType("Directory")), true, 2);
  ...
  for (int i = 0; i < RubyConfig::numberOfDirectoryPerChip(m_id); i++)  {
    m_Directory_responseFromDir_vec[i] = m_net_ptr->getToNetQueue(i+m_id*RubyConfig::numberOfDirectoryPerChip()+MachineType_base_number(string_to_MachineType("Directory")), true, 3);
    assert(m_Directory_responseFromDir_vec[i] != NULL);
    m_Directory_responseFromDir_vec[i]->setOrdering(true);
    if (FINITE_BUFFERING) {
...
Chip::~Chip()
{
  ...
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    delete m_L2Cache_L2ExpireTriggerQueue_vec[i];
  }
  for (int i = 0; i < RubyConfig::numberOfL1CachePerChip(m_id); i++)  {
...
void Chip::printBufferStats(ostream& out)
{
  ...
  // L2Cache_requestFromL2TCache
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_requestFromL2TCache_vec[i]->printStats(out);
  }
  // L2Cache_responseFromL2TCache
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_responseFromL2TCache_vec[i]->printStats(out);
  }
  // L2Cache_unblockFromL2TCache
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_unblockFromL2TCache_vec[i]->printStats(out);
  }
  // L2Cache_requestToL2TCache
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_requestToL2TCache_vec[i]->printStats(out);
  }
  // L2Cache_responseToL2TCache
  for (int i = 0; i < RubyConfig::numberOfL2CachePerChip(m_id); i++)  {
    m_L2Cache_responseToL2TCache_vec[i]->printStats(out);
  }
  // L2Cache_L2_TBEs
  // L2Cache_L2cacheMemory
  // L2Cache_L2ExpireTriggerQueue
...
void Chip::printConfig(ostream& out)
{
  ...
  out << "\nL2Cache_L2ExpireTriggerQueue numberPerChip: " << RubyConfig::numberOfL2CachePerChip() << endl;
  if(m_L2Cache_L2ExpireTriggerQueue_vec.size()>0)
    m_L2Cache_L2ExpireTriggerQueue_vec[0]->printConfig(out);
