...
void parser_t::parse(char *filename, parameters_t *parameters)
{
  ...
  char parseline[1024];
  char *option, *component, *variable, *value, *dummy;
  ...
  while(1)
  {
    line_index++;
    fflush(file);
    dummy = fgets(parseline,LINESIZE,file);
    if(/*feof(file)||*/!parseline)
      break;
    ...
    else
    {
      ...
      else
      {
        ...
        if(!stricmp(component,"-package"))
        {
          ...
          {
            fprintf(stdout,"EI ERROR (parser): pseudo package has no ID");
            exit(1);
          }
        ...
        else if(!stricmp(component,"-partition"))
        {
          ...
          {
            fprintf(stdout,"EI ERROR (parser): pseudo partition has no ID");
            exit(1);
          }
