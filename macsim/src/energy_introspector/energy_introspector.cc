...
void energy_introspector_t::configure(char *config)
{
      ...
      module_it != module.end(); module_it++)
  {
    ...
    if(module_it->second.energy_library)
    {
      ...
      fprintf(fp_config,"  unit_energy.read = %6.3lfpJ\n",unit_energy.read*1e12);
      fprintf(fp_config,"  unit_energy.write = %6.3lfpJ\n",unit_energy.write*1e12);
      if((unit_energy.read_tag > 0.0)||(unit_energy.write_tag > 0.0))
      {
        fprintf(fp_config,"  unit_energy.read_tag = %6.3lfpJ\n",unit_energy.read_tag*1e12);
        fprintf(fp_config,"  unit_energy.write_tag = %6.3lfpJ\n",unit_energy.write_tag*1e12);
      }
      if(unit_energy.search > 0.0)
        fprintf(fp_config,"  unit_energy.search = %6.3lfpJ\n",unit_energy.search*1e12);

      fprintf(fp_config,"  unit_energy.leakage = %6.3lfpJ (%3.6lf W)\n",unit_energy.leakage*1e12,unit_energy.leakage*module_it->second.energy_library->clock_frequency);
    }
    else
      fprintf(fp_config,"n/a\n");
...
{
  #ifdef EI_CONFIG_FILEOUT
  fprintf(fp_config,"pre_config processing ...\n");
  #endif
...
{
  #ifdef EI_CONFIG_FILEOUT
  fprintf(fp_config,"post_config processing...\n");
  #endif
      ...
      p_partition_it < parameters->partition.end(); p_partition_it++)
  {
    ...
    // use appropriate technology parameters
    parameters_technology_t p_technology;
...
(double time_tick, double period, string moduleID, counters_t counters)
{
  #ifdef EI_DEBUG
  ...
  if((period == 0.0)||(module_it == module.end()))
  {
  ...
  if(module_it->second.energy_library != NULL)
  {
    ...
    //energy.baseline = unit_energy.baseline*period*module_it->second.energy_library->clock_frequency;	// FIXME baseline energy
    energy.baseline = unit_energy.baseline*(double)(counters.read+counters.write);	// used by William
    ...
    energy.total = energy.baseline+energy.search\
	               +energy.read+energy.read_tag\
                   +energy.write+energy.write_tag\
				   +energy.leakage;
