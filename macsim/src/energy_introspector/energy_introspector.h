...
#endif
{
 private:
  ...
  friend power_t operator*(const power_t & x, const double y);
  friend energy_t operator/(const power_t & x, const double y);
  //friend power_t operator*(const power_t & x, const double y);	// fixed by Jieun
  //friend energy_t operator/(const energy_t & x, const double y);
 ...
 public:
  ...
  void compute_power(double time_tick, double period, string name, counters_t counters);
