...
// first find the node contain the Elem instance with the given key
    ...
    if (node->is_leaf() && node->key_count() > node->minimum_keys())
        ...
        // loop invariant: if _node_ is not reinterpret_cast<Node*> (0);, it points to a node
...
Node* Node::rotate_from_right(int parent_index_this) {
    ...
    return reinterpret_cast<Node*> (0);; // parent node still has same element count
...
// the right sibling and that parent element are then deleted
    ...
    if (mp_parent&& mp_parent->key_count() >= mp_parent->minimum_keys())
        ...
        return reinterpret_cast<Node*> (0);; // no need for parent to import an element
...
// this node and its parent element are then deleted.
    ...
    if (mp_parent==find_root() && !mp_parent->key_count()) {
        ...
        left_sib->mp_parent = reinterpret_cast<Node*> (0);;
...
void add_to_buffer(unsigned long long value, unsigned long long *buffer, unsigned long *buffer_iterator, const int NBuffer, FILE* FP) {
    assert(FP!=NULL);
    buffer[(*buffer_iterator)++] = value;

    // cout << "added to buffer: " << buffer[*buffer_iterator -1] << endl;

    if((*buffer_iterator) == NBuffer) {
        cout <<"Write to file " << buffer << " " << *buffer_iterator << endl;
        fwrite(buffer,(*buffer_iterator),sizeof(unsigned long long), FP);
        (*buffer_iterator) = 0;
    }
}


Elem& Node::profile_search (Elem& desired, Node*& last_visited_ptr, FILE* FP, unsigned long long *buffer, int NBuffer) {
    ...
    assert(FP!=NULL);
    unsigned long buffer_iterator = 0;
    Node* current = this;

    //cout << "Beginning Root: " << current << "Size: " << sizeof(current) << endl;
    if (!key_count())

        current = 0;

    // Special Marker to indicate mshr blocking. 0x0 indicates stall mshr.
    // 0x1 indicates completion of trace.
    ...
    while (current) {
        ...
        add_to_buffer((unsigned long long)current, buffer, &buffer_iterator, NBuffer, FP);
        add_to_buffer(0x0000000000, buffer, &buffer_iterator, NBuffer, FP);
        //dump marker
        ...
        if (current->m_count > 1)
        {
            ...
            //cout << "Current 1st: " << &current->m_vector[1] << endl;
            //cout << "Current last: " << &current->m_vector[m_count-1] << endl;
            add_to_buffer((unsigned long long)&current->m_vector[1], buffer, &buffer_iterator, NBuffer, FP);
            add_to_buffer((unsigned long long)&current->m_vector[m_count-1], buffer, &buffer_iterator, NBuffer, FP);
        }

        if (current->m_count>1 && desired<current->m_vector[1])
        {
            current = current->m_vector[0].mp_subtree;
            //cout << "SBTree: " << current->m_vector[0].mp_subtree << endl;
            //add_to_buffer(current->m_vector[0].mp_subtree, buffer, &buffer_iterator, NBuffer, FP);
        ...
        // if desired is greater than all values in current node
            ...
            //cout << "Current 1st: " << &current->m_vector[1] << endl;
            //cout << "Current last: " << &current->m_vector[m_count-1] << endl;
            //cout << "Current Mid: " << &current->m_vector[first+(last-first)/2] << endl;
            add_to_buffer((unsigned long long)&current->m_vector[1], buffer, &buffer_iterator, NBuffer, FP);
            add_to_buffer((unsigned long long)&current->m_vector[m_count-1], buffer, &buffer_iterator, NBuffer, FP);
            add_to_buffer((unsigned long long)&current->m_vector[first+(last-first)/2], buffer, &buffer_iterator, NBuffer, FP);
            add_to_buffer(0x0000000000, buffer, &buffer_iterator, NBuffer, FP);
            ...
            while (last-first > 1) {
                ...
                add_to_buffer((unsigned long long)&current->m_vector[mid], buffer, &buffer_iterator, NBuffer, FP);
                add_to_buffer(0x0000000000, buffer, &buffer_iterator, NBuffer, FP);
